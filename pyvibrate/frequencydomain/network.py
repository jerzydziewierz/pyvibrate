"""Network and Node classes for frequency-domain circuit topology (immutable/functional style)."""
...
class Node(NamedTuple):
    ...
    name: str
    index: int  # index in the admittance matrix (0 = ground)


class ComponentRef(NamedTuple):
    """Reference to a component for later probing."""
    name: str
    kind: str  # "R", "C", "L", "ACSource", etc.
...
class ComponentSpec(NamedTuple):
    ...
    extra_vars: int  # number of extra MNA variables (e.g., current through voltage source)
    defaults: tuple[tuple[str, float], ...] = ()  # ((param_name, default_value), ...)


class Network(NamedTuple):
    """
    Immutable circuit network topology for frequency-domain analysis.
    ...
    def compile(self) -> Solver:
        """
        Create solver functions from this network.

        Returns:
            Solver with solve_at, sweep, and probe functions
        """
        from .solver import compile_network
        return compile_network(self)
